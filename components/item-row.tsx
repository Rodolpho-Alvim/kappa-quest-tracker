...
interface ItemRowProps {
  item: QuestItem | SampleItem | RecompensaItem | TrocaItem | StreamerItem | CraftItem | HideoutItem
  sectionType:
    | "main"
    | "samples"
    | "recompensas-quests"
    ...
    return (
      <div className={`${bgColor} p-4 border-b`}>
        <div className="flex items-center justify-center">
          <span className="font-bold text-lg text-gray-700 text-center">{item.item}</span>
        </div>
      </div>
    )
  }
          ...
          <div className="flex-1 min-w-0">
            {isEditingName ? (
              ...
              <div className="flex items-start gap-2">
                <span
                  className={`font-medium text-sm leading-tight ${isCompleted ? "line-through text-gray-500" : ""}`}
                  title={item.item}
                >
                  {item.item}
                </span>
