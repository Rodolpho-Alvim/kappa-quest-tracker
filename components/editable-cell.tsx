...
export function EditableCell({ value, onSave, type, className = "", isItemName = false }: EditableCellProps) {
  ...
  if (type === "fir") {
    const bgColor = value === "Yes" ? "bg-green-200" : value === "No" ? "bg-red-200" : "bg-white"

    return (
      <div className={`${bgColor} ${className} border-r border-gray-300`}>
        <Select value={value.toString()} onValueChange={(newValue) => onSave(newValue)}>
  ...
  // Para edição inline de números e texto
  if (isEditing) {
    return (
  ...
  const canEdit = isItemName || type !== "text" || type === "number"

  return (
    <div
      className={`${bgColor} ${className} ${canEdit ? "cursor-pointer hover:bg-gray-100" : ""} p-1 text-xs min-h-[24px] flex items-center border-r border-gray-300`}
